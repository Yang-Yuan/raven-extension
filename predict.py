...
def predict_unary_default(prob, anlg, tran, d):
    ...
    for ii, opt in enumerate(prob.options):
        ...
        pred_data.append({**d, "optn": ii + 1, "pato_score": (d.get("pat_score") + score) / 2, "pred": prediction})
...
def predict_inv_unite(prob, anlg, tran, d):
    ...
    for ii, opt in enumerate(prob.options):
        ...
        if max(b5_score, opt_score) > 0.85:
            score = 0
        pred_data.append({**d, "optn": ii + 1, "pato_score": (d.get("pat_score") + score) / 2, "pred": opt})
